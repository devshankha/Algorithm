...
public class NQueenProblem {
	...
	private boolean solveNQueenOneSolutionUtil(int n, int row, Position[] positions) {
		//System.out.println("Entering for n and row "+n+" "+row);
		if (n == row) {
			//System.out.println("XXXXXXXXXXXXXXXX "+row+" "+n);
			return true;
		}
		int col;
		for (col = 0; col < n; col++) {
			...
			// check if this row and col is not under attack from any queen in previous row.
